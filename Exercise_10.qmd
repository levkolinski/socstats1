...
This `cappun ~ degree` model yields a 2102 x 5 matrix with an intercept column, and then columns for each factor of the degree variable: degreehigh school, degreeassociate/junior college, degreebachelor's , and degreegraduate.
...
comparison_table <- data.frame(Model = model_names, AIC = aic_values, BIC = bic_values)
comparison_table
```
...
mod1<-glm(cappun ~ polviews2 + weekly, data=d2_liberal_as_reference)
```
...
mod2<-glm(cappun ~ polviews2 + weekly, data=d2_moderate_as_reference)
```
...
mod3<-glm(cappun ~ polviews2 + weekly, data=d2_conservative_as_reference)
```

## Comparing the model with different reference categories

```{r}
msummary(list(mod1, mod2, mod3), gof_map = "none", output = "gt") |> 
...
The model fits the data with an AIC of 2764.853, regardless of the reference category.
...
-   $\alpha$ = 0.467
    ...
    -   The intercept value, 0.467, is the log-odds of `cappun` being 1 when the respondent is liberal and does NOT attend religious services weekly.

-   $\beta_1$ = 0.311

    -   Being conservative increases the log-odds of favoring the death penalty by 0.311

-   $\beta_2$ = 0.215

    -   Being moderate increases the log-odds of favoring the death penalty by 0.215

-   $\beta_3$ = -0.091

    -   Attending religious services weekly decreases the log-odds of favoring the death penalty by 0.091

With "moderate" as the reference category (`mod2`):

-   $\alpha$ = 0.682

    -   The intercept value, 0.682, is the log-odds of `cappun` being 1 when the respondent is moderate and does NOT attend religious services weekly.

-   $\beta_1$ = 0.096

    -   Being conservative increases the log-odds of favoring the death penalty by 0.096

-   $\beta_2$ = -0.215

    -   Being liberal decreases the log-odds of favoring the death penalty by 0.215
...
-   $\alpha$ = 0.778
    ...
    -   The intercept value, 0.778, is the log-odds of `cappun` being 1 when the respondent is conservative and does NOT attend religious services weekly.

-   $\beta_1$ = -0.096

    -   Being moderate decreaeses the log-odds of favoring the death penalty by 0.096

-   $\beta_2$ = -0.311

    -   Being liberal decreases the log-odds of favoring the death penalty by 0.311
